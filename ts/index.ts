...
let touchaudio = new Audio("se/pickup01.mp3");
let powupaudio = new Audio("se/powerup10.mp3");
let gameoveraudio = new Audio("se/nc137849.mp3");
let penaltyaudio = new Audio("se/魔王魂  レトロ22.mp3");
let bgmaudio = new Audio("se/New World (online-audio-converter.com).mp3");
let chargeaudio = new Audio("se/electric_chain.mp3")
...
let box:Box = new Box(new Vector(640,360),30,30);
let boxVector = new Vector(-1,0);

let player:Box = new Box(new Vector(150,285),150,30);
let npc:Box = new Box(new Vector(1130,210),200,30);
let block1:Box = new Box(new Vector(730,540),200,30);
let block2:Box = new Box(new Vector(730,0),200,30);
...
function draw() {
   ...
   if(box.location.x>=1250){
      ...
      speed = 0
      var end = Date.now();
      const timesec = (end - start + penaltycou*5000)/1000;
   ...
   if(box.isAttatch(block1)){
      const dy = box.getCenter().y - block1.getCenter().y
      boxVector = new Vector(-speed,0)
      boxVector = boxVector.addRotation(dy*0.5)
      touchaudio.play();
   }
   ...
   if(box.isAttatch(block2)){
      const dy = box.getCenter().y - block2.getCenter().y
      boxVector = new Vector(-speed,0)
      boxVector = boxVector.addRotation(dy*0.5)
      touchaudio.play();
   }
   ...
   if((spaceFlag == true) && (spacex == true)&&(check == true))
{
    ...
    let spaceFlagreset = function(){spacex = true; chargeaudio.play()}
...
if((spacex == false )&&(check == false))
{
   ...
   context.transform(1, 0, 0, -1, 0, canvas.height)
   let xstring = "spaceで分身発動 recharge..."+String(spacex);
   context.fillStyle = 'rgb(255,255,255)';
...
function KeyspacedownFunc(event:{keyCode:number}){
   if (event.keyCode == 32){
      spaceFlag = true;
   }
}

function KeyspaceupFunc(event:{keyCode:number}){
   if(event.keyCode == 32){
      spaceFlag = false
   }
}